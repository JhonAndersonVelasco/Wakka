...
class SettingsPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._apply_parallel_btn = QPushButton(self.tr("Aplicar"))
        self._apply_parallel_btn.clicked.connect(self._apply_parallel)
        parallel_row.addWidget(self._apply_parallel_btn)
        ...
        self._apply_ignore_btn = QPushButton(self.tr("Guardar"))
        self._apply_ignore_btn.clicked.connect(self._apply_ignored)
        ignore_row.addWidget(self._apply_ignore_btn)
        ...
        self._add_repo_btn = QPushButton(self.tr("➕ Añadir repositorio"))
        self._add_repo_btn.setObjectName("PrimaryButton")
        self._add_repo_btn.clicked.connect(self._open_add_repo_dialog)

        self._refresh_db_btn = QPushButton(self.tr("🔄 Refrescar bases de datos"))
        self._refresh_db_btn.clicked.connect(self._refresh_databases)

        self._repo_status = QLabel("")
        self._repo_status.setStyleSheet(style_subtitle(11))

        rg.addWidget(self._repo_list_widget)
        rg.addWidget(self._add_repo_btn)
        rg.addWidget(self._refresh_db_btn)
    ...
    def set_privileged_operation_running(self, running: bool):
        ...
        self._apply_parallel_btn.setEnabled(not running)
        self._apply_ignore_btn.setEnabled(not running)
        self._add_repo_btn.setEnabled(not running)
        self._refresh_db_btn.setEnabled(not running)
